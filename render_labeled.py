...
def _draw(gray, axon_labels, fiber_labels, bubble=None, alpha=0.5):
    ...
    for old, new in remap.items():
        col = PALETTE[(new - 1) % len(PALETTE)]
        for lab, c, th in [(fiber_labels == old, col, 2), (axon_labels == old, (255, 255, 255), 2)]:
            cs, _ = cv2.findContours(lab.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv2.drawContours(vis, cs, -1, c, th, cv2.LINE_AA)
